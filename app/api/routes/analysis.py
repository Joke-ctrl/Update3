...
from app.services.bar_ingest import ensure_recent_bars
...
async def run_analysis(
    ...
    # ---------------------------------------------------------------
    # Load bars for every requested timeframe, backfilling from Twelve
    # Data first (via ensure_recent_bars) if a timeframe doesn't already
    # have enough recent rows -- e.g. no MT5 EA has ever pushed bars for
    # it. If enough recent bars already exist, ensure_recent_bars is a
    # single COUNT query and does not fetch anything.
    # ---------------------------------------------------------------

    # Primary timeframe must contain enough validated bars. Computed
    # before the loop so the same floor doubles as the backfill trigger
    # threshold below.
    minimum_bars = max(
        10,
    ...
    for tf in requested_tfs:
        await ensure_recent_bars(
            db=db,
            symbol=symbol,
            timeframe=tf,
            min_bars=minimum_bars,
            outputsize=payload.bar_limit,
        )
...
#
# Allows the current Flutter APK to call:
# GET /api/v1/analysis/XAUUSD
# GET /api/v1/analysis/NAS100
# GET /api/v1/analysis/US30
# GET /api/v1/analysis/EURUSD
#
# Reuses the existing run_analysis() pipeline instead of duplicating logic.
# ---------------------------------------------------------------------------
